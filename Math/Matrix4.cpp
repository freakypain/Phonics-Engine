...
{
	set(matrix.array[0], matrix.array[1], matrix.array[2], matrix.array[3],
		matrix.array[4], matrix.array[5], matrix.array[6], matrix.array[7],
		matrix.array[8], matrix.array[9], matrix.array[10], matrix.array[11],
		matrix.array[12], matrix.array[13], matrix.array[14], matrix.array[15]);
}
...
Matrix4& Matrix4::copy(const Matrix4 &matrix )
{
	set( matrix.array[0], matrix.array[1], matrix.array[2], matrix.array[3],
			matrix.array[4], matrix.array[5], matrix.array[6], matrix.array[7],
			matrix.array[8], matrix.array[9], matrix.array[10], matrix.array[11],
...
{
	float thetaY = 0.5f * (3.14159265358979323846f * fov / 180.0f);
	float tanAY = std::tan(thetaY);
	float halfWidth = (tanAY * ratio) * near;
	float halfHeight = tanAY * near;
	...
	data[0][0] = 2.0f * near * width;
	data[0][1] = 0.0f; 
	data[0][2] = (right + left) * width; 
	data[0][3] = 0.0f;

	data[1][0] = 0.0f; 
	data[1][1] = 2.0f * near * height;
	data[1][2] = (top + bottom) * height; 
	data[1][3] = 0.0f;

	data[2][0] = 0.0f; 
	data[2][1] = 0.0f; 
	data[2][2] = -(far + near) * depth;
	data[2][3] = -2.0f * (far * near) * depth;
...
Matrix4& Matrix4::lookAt( const Vector3& eye, const Vector3& target, const Vector3& up )
{
	Vector3 x, y ,z;
	Vector3 vecUp = up;
	Vector3 vecTarget = target;


	z = (vecUp - vecTarget);
	z.operator/(z.length()).normalize();
...
Matrix4& Matrix4::lookAt2(const Vector3& eye, const Vector3& target, const Vector3& up)
{
	...
	data[0][3] = -x.dot(vecEye,x);
	data[1][3] = -y.dot(vecEye,y);
	data[2][3] = z.dot(vecEye,z);
...
{
	Vector3 coordinates = xyz;

	float x = xyz.x, y = xyz.y, z = xyz.z;
	float rAngle = 3.14159265358979323846f * angle / 180.0f;
	float c = std::cos(rAngle);
	float s = std::sin(rAngle);
	float len = coordinates.length();
	...
	data[0][0] = x * x * (1.0f - c) + c; data[0][1] = x * y * (1.0f - c) - z * s; data[0][2] = x * z * (1.0f - c) + y * s;
	data[1][0] = y * x * (1.0f - c) + z * s; data[1][1] = y * y * (1.0f - c) + c; data[1][2] = y * z * (1.0f - c) - x * s;
	data[2][0] = z * x * (1.0f - c) - y * s; data[2][1] = y * z * (1.0f - c) + x * s; data[2][2] = z * z * (1.0f - c) + c; 
...
Matrix4 Matrix4::operator*( const Matrix4 &matrix ) const
{
	...
	for (int idx = 0; idx < 4; idx++)
	{
		for (int idy = 0; idy < 4; idy++)
		{
			...
			{
				result.data[idy][idx] += data[idy][idz] * matrix.data[idz][idx];
			}
		}
	}
	return result;
